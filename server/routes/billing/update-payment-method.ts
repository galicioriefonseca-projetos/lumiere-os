...
function resolveNextDueDate(salonData: any, cycle: BillingCycle): string {
  const candidates = [
    salonData?.billing?.nextDueDate,
    salonData?.billing?.nextBillingDate,
    salonData?.nextBillingDate
  ];
...
export default async function asaasUpdatePaymentMethodHandler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (body.paymentMethod === 'pix_automatic') {
      return res.status(400).json({ error: 'Pix Automático utiliza uma jornada própria de autorização e ainda não está disponível nesta tela.' });
    }
    ...
    // Clientes que já pagaram manualmente não possuem assinatura Asaas. Nesse
    // caso, a migração cria a primeira assinatura sem antecipar a cobrança.
    if (!existingSubscriptionId) {
      ...
      if (!isManualPaid) {
        return res.status(409).json({ error: 'Esta conta ainda não possui uma assinatura Asaas nem um pagamento manual identificado. Conclua o checkout inicial antes de configurar a forma de pagamento.' });
      }
      ...
      const settings = await (billingService as any).getSettings();
      ...
      const compatible = remote.find(sub =>
        sub.status === 'ACTIVE' &&
        (sub.cycle || 'MONTHLY') === cycle &&
        Math.abs(Number(sub.value) - value) < 0.01 &&
      ...
      if (compatible) {
        await salonRef.update({
          ...
          'billing.subscriptionId': compatible.id,
          'billing.planId': planId,
          'billing.billingCycle': cycle,
          'billing.value': value,
          'billing.nextDueDate': compatible.nextDueDate,
          'billing.providerStatus': compatible.status,
          'billing.paymentMethod': compatible.billingType,
          'billing.migrationSource': 'manual_payment',
          'billing.updatedAt': new Date().toISOString(),
          asaasCustomerId: customerId,
          providerSubscriptionId: compatible.id
        });
      ...
      if (lockSnapshot.exists) {
        ...
        if (createdAt && Date.now() - createdAt < 5 * 60 * 1000) {
          return res.status(409).json({ error: 'Já existe uma tentativa de migração em andamento. Aguarde alguns segundos e tente novamente.' });
        }
      }
      await lockRef.set({ token: lockToken, salonId, planId, billingCycle: cycle, value, migration: true, createdAtMs: Date.now() });

      try {
        // Não usar invoiceUrl/checkout hospedado aqui: segundo a documentação
        // do Asaas, informar o cartão posteriormente em uma assinatura criada
        // sem cartão pode liquidar imediatamente a cobrança pendente.
        ...
        await salonRef.update({
          ...
          'billing.billingCycle': cycle,
          'billing.value': value,
          'billing.nextDueDate': sub.nextDueDate || nextDueDate,
        ...
        return res.status(200).json({
          ...
          nextDueDate: sub.nextDueDate || nextDueDate,
          billingType,
          checkoutUrl: null,
