...
export default async function changeBillingCycleHandler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!ALLOWED_CYCLES.has(billingCycle)) {
      return res.status(400).json({ error: 'Periodicidade inválida. Use MONTHLY, SEMIANNUALLY ou YEARLY.' });
    }
    ...
    if (!authResult.authorized) {
      return res.status(403).json({ error: authResult.reason || 'Sem permissão para alterar a assinatura.' });
    }

    const billing = salon.billing || {};
    if (!billing.subscriptionId) {
      return res.status(409).json({
        error: 'Esta conta ainda não possui uma assinatura Asaas ativa. Configure o pagamento antes de alterar a periodicidade.'
      });
    }

    if (String(billing.status || '').toUpperCase() !== 'ACTIVE') {
      return res.status(409).json({ error: 'A periodicidade só pode ser alterada em uma assinatura ativa.' });
    }

    const currentCycle = (billing.billingCycle || 'MONTHLY') as BillingCycle;
    if (currentCycle === billingCycle) {
      return res.status(200).json({ success: true, unchanged: true, message: 'A assinatura já utiliza esta periodicidade.' });
    }

    const result = await billingService.changeBillingCycle(salonId, billingCycle as BillingCycle);
    ...
    return res.status(Number(error?.statusCode) || 500).json({
      error: error?.message || 'Não foi possível alterar a periodicidade da assinatura.'
    });
  }
}
