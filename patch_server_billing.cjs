...
// 4. Hide Firebase/Internal errors
...
const errorTarget3 = `return res.status(500).json({ error: err.message || "Erro interno ao atualizar método de pagamento na Cakto." });`;
const errorReplacement3 = `return res.status(500).json({ error: "Erro interno ao atualizar método de pagamento na Cakto." });`;
code = code.split(errorTarget3).join(errorReplacement3);
