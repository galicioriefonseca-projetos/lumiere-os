...
export default async function createCheckoutHandler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const paymentCallback = !isDevelopmentOrPreview ? {
    ...
    const subscription = await billingService.createSubscription(
      ...
      planId,
      chosenMethod,
      { ...salonData, billing: billingData, ...(paymentCallback ? { callback: paymentCallback } : {}) },
      undefined,
      undefined,
    ...
    return res.status(200).json({
      ...
      returnUrl: paymentCallback?.successUrl || `${appUrl}/aguardando-pagamento?payment=success`,
    ...
    const asaasErrorDetail = error?.data?.errors?.[0]?.description;
    const safeMessage = asaasErrorDetail || (rawMessage.startsWith('Asaas API Error:') ? 'O Asaas recusou a criação do pagamento. Verifique a configuração da conta de pagamentos e tente novamente.' : (rawMessage || 'Erro interno ao criar pagamento.'));
