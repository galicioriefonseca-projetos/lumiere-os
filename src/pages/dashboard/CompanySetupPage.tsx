  ...
  useEffect(() => {
    ...
    const validateToken = async () => {
      try {
        ...
        if (isMounted) {
          setTokenError('Não foi possível verificar o link de configuração. Verifique sua conexão.');
        }
      } finally {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || 'Erro ao finalizar a configuração da empresa.');
      }
        ...
        <div className="w-full max-w-md bg-[#0d0d12]/90 border border-white/10 p-8 rounded-3xl shadow-2xl backdrop-blur-xl text-center">
          ...
          <div className="flex flex-col gap-2.5">
            <button
              ...
              className="w-full h-11 rounded-xl bg-[#D4AF37] hover:bg-[#c49f2c] text-black font-semibold text-sm transition"
            >
              Acessar Painel
            </button>
            <button
              onClick={() => navigate('/login')}
              className="w-full h-11 rounded-xl border border-white/10 hover:bg-white/5 text-zinc-300 text-sm transition"
            >
              Fazer Login
            </button>
          </div>
      ...
      <div className="w-full max-w-xl bg-[#0d0d12]/90 border border-white/10 p-6 sm:p-8 rounded-3xl shadow-2xl backdrop-blur-xl">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-zinc-400 mt-1 text-sm">
              Preencha os dados do seu estabelecimento para liberar o painel de gestão.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={submit} className="space-y-4">
          <label className="block space-y-1.5">
            <span className="text-xs text-zinc-400">Nome do estabelecimento</span>
            <input
              required
              value={formData.salonName}
              onChange={e => update('salonName', e.target.value)}
              placeholder="Ex: Salão Elegance & Estética"
              className="w-full h-11 rounded-xl bg-black border border-white/10 px-3 text-sm text-white placeholder-zinc-600 outline-none focus:border-[#D4AF37]"
            />
          </label>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <label className="block space-y-1.5">
              <span className="text-xs text-zinc-400">Estado (UF)</span>
              <input
                required
                ...
                placeholder="UF"
                maxLength={2}
                className="w-full h-11 rounded-xl bg-black border border-white/10 px-3 text-sm text-white placeholder-zinc-600 outline-none focus:border-[#D4AF37] uppercase"
              />
            </label>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <label className="block space-y-1.5">
              ...
              <select
                ...
                className="w-full h-11 rounded-xl bg-black border border-white/10 px-3 text-sm text-white outline-none focus:border-[#D4AF37]"
              >
            ...
            <label className="block space-y-1.5">
              <span className="text-xs text-zinc-400">Quantidade de profissionais</span>
              <select
                required
                value={formData.estimatedProfessionals}
                onChange={e => update('estimatedProfessionals', e.target.value)}
                className="w-full h-11 rounded-xl bg-black border border-white/10 px-3 text-sm text-white outline-none focus:border-[#D4AF37]"
              >
                <option value="">Selecione</option>
          ...
          <button
            disabled={loading}
            type="submit"
            className="w-full h-12 mt-4 rounded-xl bg-[#D4AF37] text-black font-bold disabled:opacity-40 hover:bg-[#c49f2c] transition flex items-center justify-center gap-2 cursor-pointer"
          >
            {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Finalizar Configuração'}
