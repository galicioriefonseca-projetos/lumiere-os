...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [demoRole, setDemoRole] = useState<Role | null>(() => {
    try {
      return DEMO_MODE_ENABLED ? sessionStorage.getItem('demo_role') as Role | null : null;
    } catch (e) {
      return null;
    }
  });
...
- offlineError: ${info.offlineError}`);
  ...
  const runDemoBootstrapFallback = async (uid: string, email: string, displayName: string | null) => {
    if (!DEMO_MODE_ENABLED || !DEMO_USER_EMAIL || email !== DEMO_USER_EMAIL) {
      throw new Error('Modo de demonstração desativado.');
    }
    console.log("[Demo Bootstrap] Iniciando para", email, "UID:", uid);
    ...
    try {
      ...
      console.log("[Demo Bootstrap] Processo de bootstrap operado com sucesso de ponta a ponta!");
      return { uData, demoSalonId };
    } catch (err) {
      console.error("[Demo Bootstrap] Erro no bootstrap de Administrador de Demonstração:", err);
      throw err;
    }
  };
  ...
  const fetchUserData = async (uid: string) => {
    ...
    try {
      ...
      if (DEMO_MODE_ENABLED && currentUser?.email === DEMO_USER_EMAIL) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(getAuth(), async (user) => {
      ...
      if (user) {
        ...
        // Bootstrap de demonstração, habilitado somente por variável explícita.
        if (DEMO_MODE_ENABLED && user.email === DEMO_USER_EMAIL) {
          try {
  ...
  const signInWithGoogle = async (): Promise<AuthUser> => {
    ...
    const isDemoOwner = DEMO_MODE_ENABLED && user.email === DEMO_USER_EMAIL;
    ...
    <AuthContext.Provider value={{
      ...
      {loading ? (
        <div className="min-h-[100dvh] flex flex-col items-center justify-center bg-background text-foreground">
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#D4AF37" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="animate-spin mb-6">
          ...
          <p className="text-zinc-500 font-medium">Verificando sessão...</p>
        </div>
      ) : children}
