      ...
      collection: (colName: string) => {
        ...
        return {
          doc: (id: string) => ({
            ...
            collection: () => ({
              doc: () => ({ id: "history_123", set: mockHistorySet })
            })
          })
        };
      }
...
describe("Testes de Segurança de Faturamento (Billing Security)", () => {
  ...
  beforeEach(() => {
    ...
    mockSalonGet.mockResolvedValue({ exists: false, data: () => ({}) });
  });
  ...
  describe("14 Casos de Negócio e Segurança de Faturamento", () => {
    ...
    it("13. O webhook Cakto deve processar uma cobrança aprovada com oferta oficial sem expor falhas internas", async () => {
      const webhookHandler = (await import("./webhook.js")).default;
      const req: any = {
        method: "POST",
        headers: {
      ...
      expect(mockSalonUpdate).toHaveBeenCalledWith(expect.objectContaining({
        plan: "start",
        paymentStatus: "paid",
        subscriptionStatus: "active"
      }));
    ...
    it("15. O webhook só promove onboarding existente após pagamento aprovado e preserva o ownerId autenticado", async () => {
      const webhookHandler = (await import("./webhook.js")).default;
      const req: any = {
        method: "POST",
        headers: { "x-cakto-token": "super_secret_token" },
        body: {
          event: "purchase_approved",
          offer_id: "off_start",
          external_id: "salon_new_customer",
          current_period_end: "2026-08-15T00:00:00.000Z"
        }
      };
      const res = createMockRes();
      process.env.CAKTO_WEBHOOK_SECRET = "super_secret_token";
      process.env.NODE_ENV = "production";
      mockSalonGet.mockResolvedValueOnce({ exists: false, data: () => ({}) });
      mockOnboardingGet.mockResolvedValueOnce({
        exists: true,
        data: () => ({
          ownerId: "owner_new",
          ...
          name: "Salão Novo",
          pendingPlan: "start",
          pendingOfferId: "off_start",
          pendingCheckoutEmail: "billing@example.com",
    ...
    it("16. O webhook não cria salão arbitrário quando o onboarding não existe", async () => {
      const webhookHandler = (await import("./webhook.js")).default;
      const req: any = {
        method: "POST",
        headers: { "x-cakto-token": "super_secret_token" },
      ...
      process.env.NODE_ENV = "production";
      mockSalonGet.mockResolvedValueOnce({ exists: false, data: () => ({}) });
      mockOnboardingGet.mockResolvedValueOnce({ exists: false });
